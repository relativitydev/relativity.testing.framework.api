...
namespace Relativity.Testing.Framework.Api.Tests.Strategies.Imaging.Job
{
	...
	public class ImagingJobSubmitSingleDocumentStrategyV1Fixture
	{
		...
		private readonly ImagingJobSubmitSingleDocumentRequest _imagingJobSubmitSingleDocumentRequest = new ImagingJobSubmitSingleDocumentRequest();
		...
		{
			_sut.SubmitSingleDocument(_WORKSPACE_ID, _DOCUMENT_ID, _imagingJobSubmitSingleDocumentRequest);
		...
		{
			await _sut.SubmitSingleDocumentAsync(_WORKSPACE_ID, _DOCUMENT_ID, _imagingJobSubmitSingleDocumentRequest).ConfigureAwait(false);
			_mockArtifactIdValidator.Verify(x => x.Validate(_DOCUMENT_ID, "Document"), Times.Once);
			_mockWorkspaceIdValidator.Verify(x => x.Validate(_WORKSPACE_ID), Times.Once);
		}
	}

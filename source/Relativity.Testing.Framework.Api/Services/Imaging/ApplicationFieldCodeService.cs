...
namespace Relativity.Testing.Framework.Api.Services.Imaging
{
	internal class ApplicationFieldCodeService : IApplicationFieldCodeService
	{
		...
		{
			return _applicationFieldCodeCreateStrategy.Create(workspaceId, applicationFieldCode);
		}

		public async Task<ApplicationFieldCode> CreateAsync(int workspaceId, ApplicationFieldCode applicationFieldCode)
		{
			return await _applicationFieldCodeCreateStrategy.CreateAsync(workspaceId, applicationFieldCode).ConfigureAwait(false);
		}
		...
		{
			return _applicationFieldCodeUpdateStrategy.Update(workspaceId, applicationFieldCode);
		}

		public async Task<ApplicationFieldCode> UpdateAsync(int workspaceId, ApplicationFieldCode applicationFieldCode)
		{
			return await _applicationFieldCodeUpdateStrategy.UpdateAsync(workspaceId, applicationFieldCode).ConfigureAwait(false);
		}
	}
}
